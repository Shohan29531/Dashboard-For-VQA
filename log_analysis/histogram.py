...
# Define model pairs
models = ['GT_N', 'Random', 'BLIP', 'BLIP@Shadow', 'GPV-1', 'GPV-1@Shadow', 'GPT4V', 'GPT4V@Shadow']
...
for model in models:
    ...
    count = df_model['score'].value_counts().sort_index().to_dict()

    # Ensure x_data and y_data have a length of 11
    x_data = [i for i in range(11)]
    y_data = [count[i] if i in count else 0 for i in range(11)]
